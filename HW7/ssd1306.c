...
void ssd1306_setup() {
    ...
    ssd1306_update(); // write cleared buffer to screen, screen is now off and ready to use
}
...
void drawChar(int x, int y, char letter) {
    int i;
    for (i=0; i<5; i++) { // may have to start at 5 and go to 0
        char c = ASCII[letter-32][i];
        for (int j = 0; j<8; j++) { // may have to start at 8 and go to 0
            char bit = (c>>j)&0b1;
...
void drawMessage(int x,int y,char * m) {
    int i =0;
    while(m[i]) { // loop until you encounter null character (0)
